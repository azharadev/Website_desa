...
class suratonline extends CI_Controller
{
    function __construct()
    {
        ...
        $this->load->model('m_suratonline');
    }

    function index()
    {
        $this->m_suratonline->update_status_suratonline();
        $x['data'] = $this->m_suratonline->get_all_suratonline();
        $this->load->view('admin/v_suratonline', $x);
    }

    function hapus_suratonline()
    {
        $kode = $this->input->post('kode');
        $this->m_suratonline->hapus_suratonline($kode);
        echo $this->session->set_flashdata('msg', 'success-hapus');
        redirect('admin/suratonline');
    }

    function export_ex()
    {
        $surat = $this->m_suratonline->listing();
        // var_dump($surat);
        // exit;
        $data = array(
            'title' => 'Data surat',
        ...
        foreach ($data['surat'] as $data) {
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('B' . $baris, $data->suratonline_nama);
            $objPHPExcel->getActiveSheet()->setCellValue('C' . $baris, $data->suratonline_email);
            $objPHPExcel->getActiveSheet()->setCellValue('D' . $baris, $data->suratonline_suratonline);
            $objPHPExcel->getActiveSheet()->setCellValue('E' . $baris, $data->suratonline_pesan);
    ...
    {
        $surat = $this->m_suratonline->get_all();
        // $tanggal = date('d-m-Y');
    ...
    {
        $pdf->Cell(8, 15, $no, 1, 0, 'C');
        $pdf->Cell(27, 15, $surat['suratonline_nama'], 1, 0, 'C');
        $pdf->Cell(35, 15, $surat['suratonline_email'], 1, 0, 'C');
        $pdf->Cell(25, 15, $surat['suratonline_suratonline'], 1, 0, 'C');
        $pdf->Cell(95, 15, $surat['suratonline_pesan'], 1, 1, 'C');
    }
}
